...
For `log_type` specifications, see [log types](https://localhost:3001/get-started/observability_data_model#log-types).
</Note>

## Body parameters

<ParamField path="unique_id" type="string">
You can get a log's `unique_id` from the [Logs list endpoint](/api-endpoints/observe/logs/logs-list-endpoint). Then pass this `unique_id` to update the log.
```json
{
    "unique_id": "xxxxxx"
}
