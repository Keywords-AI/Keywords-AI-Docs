...
For complete `log_type` specifications, see [log types](https://localhost:3001/get-started/observability_data_model#log-types).
...
For complete specifications of each type, see [log types](https://localhost:3001/get-started/observability_data_model#log-types).
</Accordion>
</ParamField>
...
An identifier for the customer that invoked this request. Helps with visualizing user activities. See [customer identifier details](/features/generation/customer-identifier).
...
ID of the prompt template used. See [Prompts documentation](/features/prompt/deploy-a-prompt#1-find-the-prompt-id).
</ParamField>
