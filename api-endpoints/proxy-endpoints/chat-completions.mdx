...
params ={"messages": [{"role": "user", "content": "Hello"}], "models": ["gpt-3.5-turbo", "gpt-3.5-turbo-16k"], "stream": False, "max_tokens": 100}
...
<ParamField path="temperature" type="number" default={1}>
  ...
  a more random response. See more info on the [LLM parameters
...
<ParamField path="exclude_models" type="array" default={[]}>
  ...
  This only excludes models in the router, if `model` parameter will take precedence over this parameter, and`fallback_models` and [safety net](/platform-features/alerts-and-fallback.mdx) will still use the excluded models to catch failures.
</ParamField>
