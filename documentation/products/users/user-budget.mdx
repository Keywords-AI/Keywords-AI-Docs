...
You can also set the budget to a specific user by calling the [User Update endpoint](/api-endpoints/observe/users/user-update-endpoint) or the [User Creation endpoint](/api-endpoints/observe/users/user-creation-endpoint) for the new users.