...
### Via LLM Gateway
If you are using the [LLM Gateway](/documentation/products/gateway/quickstart), you should first [set up your prompts](/documentation/products/prompt_management/use_prompt_in_code/use_prompt_in_llms) in the code. Then we will log the LLM requests and variables automatically.
...
When you make a request through the [Logging ingestion](/documentation/api-endpoints/integration/request-logging-endpoint), you can send the prompt variables in the `prompt_messages` field. Just simply wrap your prompt variables in pairs of `{{}}`.
