...
Specify the list of models the router to chooose between. If not specified, <strong>all models</strong> will be used. See the list of model [here](/integration/supported-models)
</ParamField>

<ParamField path="fallback_models" type="array[string]">
Specify the list of backup models (ranked by priority) to response in case of a failure in the primary model. See the list of model [here](/integration/supported-models)
</ParamField>


<ParamField path="customer_api_keys" type="dict">
You can pass in a dictionary of your customer's API keys for specific models. If the router selects a model that is in the dictionary, it will attempt to use the customer's API key for calling the model before using your [integration API key](/integration/llm-credit) or Keywords AI's default API key.
...
<ParamField path="temperature" type="float" default={1}>
  ...
  a more random response. See more info in the [LLM parameters page](/get-started/llm-params)
</ParamField>
...
<ParamField path="frequency_penalty" type="number">
  ...
  so far. Decreases the model's likelihood to repeat the same line verbatim
</ParamField>
...
<ParamField path="disable_log" type="boolean" deafault={false}>
  ...
  and output messages will be ommited from the log.
</ParamField>
