...
```python
from google.Gen AI import types
...
```typescript
import { GoogleGen AI } from "@google/Gen AI";

const Gen AI = new GoogleGen AI({
...
```typescript
async function run() {
  const result = await Gen AI.models.generateContent({
    model: "gemini-2.5-flash",
...
// Example: Comprehensive GenerateContentConfig showcasing various parameters
async function run() {
  const response = await Gen AI.models.generateContent({
    model: "gemini-2.5-flash",
    contents: [
      { 
        role: "user", 
