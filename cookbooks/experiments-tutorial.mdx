...
Use the [Log ingestion](/api-endpoints/integration/request-logging-endpoint) endpoint to send your LLM request data to Keywords AI.
...
Once you have logs, you might want to filter them to create a dataset. Use the [Retrieve log list](/api-endpoints/async-endpoints/logs-list-endpoint) endpoint to find the specific logs you want to include (e.g., by time range or metadata).
...
Refer to the [Create dataset](/api-endpoints/evaluation/dataset/create) endpoint for more details.
...
See [Create Evaluator](/api-endpoints/evaluation/evaluators/create) for options.
...
If you are running a **Custom Workflow** (where you process inputs yourself and submit results), follow the [Experiment V2 API](/api-endpoints/experiments/create).
...
1. [List the logs](/api-endpoints/experiments/list-logs) to get the inputs.
2. Process the inputs with your model/logic.
3. [Submit the results](/api-endpoints/experiments/submit-results) back to the experiment.
