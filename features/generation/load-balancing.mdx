...
description: "Balance the load of your requests between different models."
---

Load balancing is a feature that allows you to balance the request load across different models provided by different providers. You could specify weights for each provider based on their rate limit and your preference.

_See all supported params [here](/api-endpoints/proxy-endpoints/chat-completions)._

## Load balancing example

You should use a valid model name from the list of supported models as the name of a load-balancing group. You can find the list of supported models [here](https://platform.keywordsai.co/platform/models?sort_by=model_name). When you call a model name that is also a load-balancing group name, the router will trigger the load-balancing logic.
...
<ParamField path="model" type="string" required>
  ...
  <br/>For the example above, the model is `gpt-3.5-turbo`, but `gpt-3.5-turbo` is not included in the load balancing group, so it becomes a group name and it will not be called.
</ParamField>
<ParamField path="weight" type="integer" required>
    Specify the weight of the model (has to be a positive integer). The higher the weight, the more requests will be sent to the model. <br/>
    For the example above, the percentage of requests sent to `claude-3-5-sonnet-20240620` and `azure/gpt-35-turbo` is 50% each.
</ParamField>
